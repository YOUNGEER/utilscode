...
public class StatusBarFontHelper {
    ...
    public static int setStatusBarMode(Activity activity, boolean isFontColorDark) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                result = ANDROID_M;
            } else if (new MIUIHelper().setStatusBarLightMode(activity, isFontColorDark)) {
                result = MIUI;
    ...
    public static void setStatusBarMode(Activity activity, @SystemType int type, boolean isFontColorDark) {
        if (null == activity) {
            return;
        }
        if (type == ANDROID_M) {
